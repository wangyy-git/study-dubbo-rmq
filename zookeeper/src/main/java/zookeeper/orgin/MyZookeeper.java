...
public class MyZookeeper implements Watcher {
    ...
    public static void main(String[] args) {
        try {
            zk = new ZooKeeper(IP,TIME_OUT,new MyZookeeper());
...
//            zk.create("/demo", "demo".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            ...
            System.out.println("=============================");
            //getData
            //注册监听器是用来做回调函数 触发的是传入的监听器
            
//            参数传Boolean值 watch ? this.watchManager.defaultWatcher : null
            System.out.println(new String(zk.getData("/java",
                    //回调函数：只要对该节点进行改变就会触发
                    //然后仍然会回来执行这个函数 但是线程要一直在？
                    //但原生zk client 的watch只能触发一次
                    watchedEvent -> {
...
//            System.out.println(new String(zk.getData("/java",true, stat)));
            zk.getData("/java", false, new AsyncCallback.DataCallback() {
                @Override
